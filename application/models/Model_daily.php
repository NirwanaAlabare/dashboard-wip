...
class Model_daily extends CI_Model
{
    ...
    function cari_harikerja()
    {
        $db_sblokal = $this->load->database('db_sblokal', TRUE);
        $query = $db_sblokal->query("SELECT GROUP_CONCAT(tanggal) tanggal from (select SUBSTR(tanggal,9,2) tanggal from dim_date where bulan_text = DATE_FORMAT(CURRENT_DATE, '%m') and status_prod = 'KERJA' and tahun = DATE_FORMAT(CURRENT_DATE, '%Y')) a");
    ...
    function cari_effi_factory()
    {
        $db_sblokal = $this->load->database('db_sblokal', TRUE);
        $query = $db_sblokal->query("SELECT effi,target_effy,per_rft from dsb_data_factory");
            foreach ($query->result() as $data) {
                $hasil = ($data->effi);
            }
    ...
    function cari_target_effi_factory()
    {
        $db_sblokal = $this->load->database('db_sblokal', TRUE);
        $query = $db_sblokal->query("SELECT effi,target_effy,per_rft from dsb_data_factory");
            foreach ($query->result() as $data) {
...
(SELECT b.sewing_line,- count(a.id) deffect from output_defects a inner join master_plan b on b.id = a.master_plan_id where tgl_plan = CURRENT_DATE() GROUP BY b.sewing_line) b on b.sewing_line = a.sewing_line) a) c on c.sewing_line = a.sewing_line) a order by per_rft desc) b on b.sewing_line = line.username order by b.effi desc limit 5");
    ...
    function cari_effi_chiefline($id)
    {
        $db_sblokal = $this->load->database('db_sblokal', TRUE);
        $query = $db_sblokal->query("SELECT GROUP_CONCAT(effi) effi_line  from (SELECT IF(effi is null,'''''',effi) effi ,id_chief from (select SUBSTR(tanggal,9,2) tanggal_s, tanggal from dim_date where bulan_text = DATE_FORMAT(CURRENT_DATE, '%m') and status_prod = 'KERJA' and tahun = DATE_FORMAT(CURRENT_DATE, '%Y')) a
left join
(select id_chief,a.tgl_plan, Round((min_prod/if(a.tgl_plan = CURRENT_DATE(),min_tersedia_now,min_tersedia) * 100),2) effi from (select tgl_plan,id_chief, SUM(min_prod) min_prod from (SELECT c.id_chief,tgl_plan, master_plan_id,smv, count(a.id) actual, ROUND((count(a.id) * smv),4) min_prod from output_rfts a inner join master_plan b on b.id = a.master_plan_id inner join master_lead_line c on c.sewing_line  = b.sewing_line where tgl_plan BETWEEN (select tgl_awal from tbl_tgl_tb where bulan = DATE_FORMAT(CURRENT_DATE, '%m') and tahun = DATE_FORMAT(CURRENT_DATE, '%Y')) and (select tgl_akhir from tbl_tgl_tb where bulan = DATE_FORMAT(CURRENT_DATE, '%m') and tahun = DATE_FORMAT(CURRENT_DATE, '%Y')) and c.id_chief = '$id' GROUP BY master_plan_id
) a GROUP BY a.tgl_plan) a left join
(select tgl_plan, sum(min_tersedia) min_tersedia from (select tgl_plan,ROUND((jam_kerja * 60) * man_power,2) min_tersedia from (select tgl_plan, id_ws,sum(jam_kerja) jam_kerja, MAX(man_power) man_power from master_plan a inner join master_lead_line b on b.sewing_line  = a.sewing_line where tgl_plan BETWEEN (select tgl_awal from tbl_tgl_tb where bulan = DATE_FORMAT(CURRENT_DATE, '%m') and tahun = DATE_FORMAT(CURRENT_DATE, '%Y')) and (select tgl_akhir from tbl_tgl_tb where bulan = DATE_FORMAT(CURRENT_DATE, '%m') and tahun = DATE_FORMAT(CURRENT_DATE, '%Y')) and b.id_chief = '$id' GROUP BY tgl_plan,a.sewing_line) a) a GROUP BY a.tgl_plan) b on b.tgl_plan = a.tgl_plan
left join 
...
(SELECT sewing_line,COALESCE(sum(jam_kerja),0) as jam_kerja,COALESCE(sum(plan_target),0) as plan_target from master_plan where tgl_plan = CURRENT_DATE() GROUP BY sewing_line) b on b.sewing_line = a.sewing_line) a) c on c.tgl_plan = a.tgl_plan) b on b.tgl_plan = a.tanggal) a");
    ...
    function cari_rft_chiefline($id)
    {
        $db_sblokal = $this->load->database('db_sblokal', TRUE);
        $query = $db_sblokal->query("SELECT GROUP_CONCAT(per_rft) per_rft_line from (SELECT id_chief,IF(per_rft is null,'''''',per_rft) per_rft from (select SUBSTR(tanggal,9,2) tanggal_s, tanggal from dim_date where bulan_text = DATE_FORMAT(CURRENT_DATE, '%m') and status_prod = 'KERJA' and tahun = DATE_FORMAT(CURRENT_DATE, '%Y')) a
left join
(SELECT id_chief,tgl_plan, round((rft / actual * 100),2) per_rft from (SELECT id_chief,a.tgl_plan, COALESCE(rft,0) rft ,(COALESCE(rft,0) + COALESCE(deffect,0)) actual, COALESCE(deffect,0) defect from (SELECT c.id_chief,b.tgl_plan, count(a.id) rft from output_rfts a inner join master_plan b on b.id = a.master_plan_id inner join master_lead_line c on c.sewing_line  = b.sewing_line where a.status = 'NORMAL' and b.tgl_plan BETWEEN (select tgl_awal from tbl_tgl_tb where bulan = DATE_FORMAT(CURRENT_DATE, '%m') and tahun = DATE_FORMAT(CURRENT_DATE, '%Y')) and (select tgl_akhir from tbl_tgl_tb where bulan = DATE_FORMAT(CURRENT_DATE, '%m') and tahun = DATE_FORMAT(CURRENT_DATE, '%Y')) and c.id_chief = '$id' GROUP BY b.tgl_plan) a left join 
(SELECT b.tgl_plan, count(a.id) deffect from output_defects a inner join master_plan b on b.id = a.master_plan_id inner join master_lead_line c on c.sewing_line  = b.sewing_line where b.tgl_plan BETWEEN (select tgl_awal from tbl_tgl_tb where bulan = DATE_FORMAT(CURRENT_DATE, '%m') and tahun = DATE_FORMAT(CURRENT_DATE, '%Y')) and (select tgl_akhir from tbl_tgl_tb where bulan = DATE_FORMAT(CURRENT_DATE, '%m') and tahun = DATE_FORMAT(CURRENT_DATE, '%Y')) and c.id_chief = '$id' GROUP BY b.tgl_plan) b on b.tgl_plan = a.tgl_plan) a) b on b.tgl_plan = a.tanggal) a");
            foreach ($query->result() as $data) {
    ...
    {
        $db_sblokal = $this->load->database('db_sblokal', TRUE);
        $query = $db_sblokal->query("SELECT CEIL(COUNT(a.id)/4) jml_slide from (select a.id,a.sewing_line,a.sewing_name,leader_name,short_name from master_lead_line a inner join master_plan b on b.sewing_line = a.sewing_line where b.cancel != 'Y' GROUP BY a.sewing_line order by a.id asc) a");
            foreach ($query->result() as $data) {
    ...
    function cari_rft_line($line)
    {
        $db_sblokal = $this->load->database('db_sblokal', TRUE);
        $query = $db_sblokal->query("SELECT sewing_line,GROUP_CONCAT(per_rft) per_rft_line from (SELECT IF(per_rft is null,'''''',per_rft) per_rft,sewing_line from (select SUBSTR(tanggal,9,2) tanggal_s, tanggal from dim_date where bulan_text = DATE_FORMAT(CURRENT_DATE, '%m') and status_prod = 'KERJA' and tahun = DATE_FORMAT(CURRENT_DATE, '%Y')) a
left join
(SELECT sewing_line,tgl_plan, round((rft / actual * 100),2) per_rft from (SELECT sewing_line,a.tgl_plan, COALESCE(rft,0) rft ,(COALESCE(rft,0) + COALESCE(deffect,0)) actual, COALESCE(deffect,0) defect from (SELECT sewing_line,b.tgl_plan, count(a.id) rft from output_rfts a inner join master_plan b on b.id = a.master_plan_id where a.status = 'NORMAL' and b.tgl_plan BETWEEN (select tgl_awal from tbl_tgl_tb where bulan = DATE_FORMAT(CURRENT_DATE, '%m') and tahun = DATE_FORMAT(CURRENT_DATE, '%Y')) and (select tgl_akhir from tbl_tgl_tb where bulan = DATE_FORMAT(CURRENT_DATE, '%m') and tahun = DATE_FORMAT(CURRENT_DATE, '%Y')) and b.sewing_line = '$line' GROUP BY b.tgl_plan) a left join 
(SELECT b.tgl_plan, count(a.id) deffect from output_defects a inner join master_plan b on b.id = a.master_plan_id where b.tgl_plan BETWEEN (select tgl_awal from tbl_tgl_tb where bulan = DATE_FORMAT(CURRENT_DATE, '%m') and tahun = DATE_FORMAT(CURRENT_DATE, '%Y')) and (select tgl_akhir from tbl_tgl_tb where bulan = DATE_FORMAT(CURRENT_DATE, '%m') and tahun = DATE_FORMAT(CURRENT_DATE, '%Y')) and b.sewing_line = '$line' GROUP BY b.tgl_plan) b on b.tgl_plan = a.tgl_plan) a) b on b.tgl_plan = a.tanggal) a");
            foreach ($query->result() as $data) {
                $hasil = ($data->per_rft_line);
            }
    ...
    function cari_effi_line($line)
    {
        $db_sblokal = $this->load->database('db_sblokal', TRUE);
        $query = $db_sblokal->query("SELECT sewing_line,GROUP_CONCAT(effi) effi_line from (SELECT IF(effi is null,'''''',effi) effi,sewing_line from (select SUBSTR(tanggal,9,2) tanggal_s, tanggal from dim_date where bulan_text = DATE_FORMAT(CURRENT_DATE, '%m') and status_prod = 'KERJA' and tahun = DATE_FORMAT(CURRENT_DATE, '%Y')) a
left join
(select sewing_line,a.tgl_plan, Round((min_prod/if(a.tgl_plan = CURRENT_DATE(),min_tersedia_now,min_tersedia) * 100),2) effi from (select sewing_line,tgl_plan, SUM(min_prod) min_prod from (SELECT sewing_line,tgl_plan, master_plan_id,smv, count(a.id) actual, ROUND((count(a.id) * smv),4) min_prod from output_rfts a inner join master_plan b on b.id = a.master_plan_id where tgl_plan BETWEEN (select tgl_awal from tbl_tgl_tb where bulan = DATE_FORMAT(CURRENT_DATE, '%m') and tahun = DATE_FORMAT(CURRENT_DATE, '%Y')) and (select tgl_akhir from tbl_tgl_tb where bulan = DATE_FORMAT(CURRENT_DATE, '%m') and tahun = DATE_FORMAT(CURRENT_DATE, '%Y')) and b.sewing_line = '$line' GROUP BY master_plan_id) a GROUP BY a.tgl_plan) a left join
(select tgl_plan, sum(min_tersedia) min_tersedia from (select tgl_plan,ROUND((jam_kerja * 60) * man_power,2) min_tersedia from (select tgl_plan, id_ws,sum(jam_kerja) jam_kerja, MAX(man_power) man_power from master_plan where tgl_plan BETWEEN (select tgl_awal from tbl_tgl_tb where bulan = DATE_FORMAT(CURRENT_DATE, '%m') and tahun = DATE_FORMAT(CURRENT_DATE, '%Y')) and (select tgl_akhir from tbl_tgl_tb where bulan = DATE_FORMAT(CURRENT_DATE, '%m') and tahun = DATE_FORMAT(CURRENT_DATE, '%Y')) and sewing_line = '$line' GROUP BY tgl_plan,sewing_line) a) a GROUP BY a.tgl_plan) b on b.tgl_plan = a.tgl_plan
left join 
