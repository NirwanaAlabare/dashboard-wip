...
class Model_factory_before extends CI_Model
{
    ...
    function cari_defectno1()
    {
        ...
        $query = $db_sblokal->query("SELECT defect_type from (select id,CONCAT(UCASE(LEFT(defect_type, 1)),SUBSTRING(defect_type, 2)) defect_type ,deffect from (select                         a.defect_type_id,count(a.id) deffect from output_defects a
                                     ...
                                     where DATE_FORMAT(created_at, '%Y-%m-%d') = '2023-06-26'
    ...
    {
        $db_sblokal = $this->load->database('db_sblokal', TRUE);
        $query = $db_sblokal->query("SELECT smv from master_plan where tgl_plan = '2023-06-26' and sewing_line = 'line_17'");
    ...
    {
        $db_sblokal = $this->load->database('db_sblokal', TRUE);
        $query = $db_sblokal->query("SELECT man_power from master_plan where tgl_plan = '2023-06-26' and sewing_line = 'line_17'");
        foreach ($query->result() as $data) {
    ...
    {
        $db_sblokal = $this->load->database('db_sblokal', TRUE);
        $query = $db_sblokal->query("SELECT count(a.id) actual from output_rfts a inner join master_plan b on b.id = a.master_plan_id where b.sewing_line = 'line_17' and DATE_FORMAT(created_at, '%Y-%m-%d') = '2023-06-26'");
            foreach ($query->result() as $data) {
                $hasil = ($data->actual);
            }
    ...
    {
        $db_sblokal = $this->load->database('db_sblokal', TRUE);
        $query = $db_sblokal->query("SELECT COALESCE(sum(jam_kerja),0) as jam_kerja from master_plan where tgl_plan = '2023-06-26' and sewing_line = 'line_17'");
            foreach ($query->result() as $data) {
    ...
    {
        $db_sblokal = $this->load->database('db_sblokal', TRUE);
        $query = $db_sblokal->query("SELECT ROUND(AVG(effi),2) per_effi from (select nama_line,COALESCE(effi,0) effi,COALESCE(per_rft,0) per_rft,COALESCE(per_defect,0) per_defect from (select username,SUBSTR(FullName FROM 8) nama_line from userpassword where Groupp = 'sewing' order by username asc
) line inner join 
(SELECT sewing_line,effi,per_rft,per_defect from (SELECT a.sewing_line,plan_target,actual,rft,deffect,effi,ROUND((rft / actual * 100),2) per_rft, ROUND((deffect / actual * 100),2) per_defect FROM (select sewing_line,min_prod,(man_power * menit_real) min_tersedia, actual,plan_target, ROUND((min_prod / (man_power * menit_real) * 100),2) effi from (Select a.sewing_line,actual,min_prod,man_power,if(menit_real > (jam_kerja * 60),jam_kerja,jam_real) jam_real, if(menit_real > (jam_kerja * 60),(jam_kerja * 60),menit_real) menit_real, jam_kerja,plan_target from (SELECT a.sewing_line,sum(actual) actual,sum(min_prod) min_prod,man_power,jam_real,menit_real FROM (SELECT id,sewing_line,smv,actual,round(smv * actual,4) min_prod FROM (SELECT sewing_line,id,smv from master_plan where tgl_plan = '2023-06-26' ) a inner join 
(SELECT master_plan_id, count(a.id) actual from output_rfts a inner join master_plan b on b.id = a.master_plan_id where DATE_FORMAT(created_at, '%Y-%m-%d') = '2023-06-26'GROUP BY master_plan_id) b on b.master_plan_id = a.id) a inner join 
(SELECT sewing_line,man_power,jam_masuk,jam_sekarang, if(jam_kerja >= '6',jam_kerja -1,jam_kerja) jam_real, if(jam_kerja >= '6',menit_kerja -60,menit_kerja) menit_real from (SELECT sewing_line, man_power, CONCAT('2023-06-26',' ','07:00:00') jam_masuk,'2023-06-26' as jam_sekarang, TIMESTAMPDIFF(hour,CONCAT('2023-06-26',' ','07:00:00'),'2023-06-26 20:00:00') jam_kerja,TIMESTAMPDIFF(minute,CONCAT('2023-06-26',' ','07:00:00'),'2023-06-26 20:00:00') menit_kerja from master_plan where tgl_plan = '2023-06-26' GROUP BY sewing_line order by sewing_line asc ) a) b on b.sewing_line = a.sewing_line GROUP BY a.sewing_line) a inner join
(SELECT sewing_line,COALESCE(sum(jam_kerja),0) as jam_kerja,COALESCE(sum(plan_target),0) as plan_target from master_plan where tgl_plan = '2023-06-26' GROUP BY sewing_line) b on b.sewing_line = a.sewing_line) a) a left join 
(SELECT b.sewing_line,count(a.id) deffect from output_defects a inner join master_plan b on b.id = a.master_plan_id where DATE_FORMAT(created_at, '%Y-%m-%d') = '2023-06-26' GROUP BY b.sewing_line) b on b.sewing_line = a.sewing_line LEFT JOIN
(select sewing_line,(rft + deffect) rft from (select a.sewing_line,COALESCE(rft,0) rft, COALESCE(deffect,0) deffect from (SELECT b.sewing_line,count(a.id) rft from output_rfts a inner join master_plan b on b.id = a.master_plan_id where DATE_FORMAT(created_at, '%Y-%m-%d') = '2023-06-26' and status = 'NORMAL' GROUP BY b.sewing_line) a left join
(SELECT b.sewing_line,- count(a.id) deffect from output_defects a inner join master_plan b on b.id = a.master_plan_id where DATE_FORMAT(created_at, '%Y-%m-%d') = '2023-06-26' and a.defect_status = 'defect' GROUP BY b.sewing_line) b on b.sewing_line = a.sewing_line) a) c on c.sewing_line = a.sewing_line) a order by per_rft desc) b on b.sewing_line = line.username order by b.effi desc ) a");
            foreach ($query->result() as $data) {
                $hasil = ($data->per_effi);
            }
    ...
    {
        $db_sblokal = $this->load->database('db_sblokal', TRUE);
        $query = $db_sblokal->query("SELECT ROUND(AVG(per_rft),2) per_rft from (select nama_line,COALESCE(effi,0) effi,COALESCE(per_rft,0) per_rft,COALESCE(per_defect,0) per_defect from (select username,SUBSTR(FullName FROM 8) nama_line from userpassword where Groupp = 'sewing' order by username asc
) line inner join 
(SELECT sewing_line,effi,per_rft,per_defect from (SELECT a.sewing_line,plan_target,actual,rft,deffect,effi,ROUND((rft / actual * 100),2) per_rft, ROUND((deffect / actual * 100),2) per_defect FROM (select sewing_line,min_prod,(man_power * menit_real) min_tersedia, actual,plan_target, ROUND((min_prod / (man_power * menit_real) * 100),2) effi from (Select a.sewing_line,actual,min_prod,man_power,if(menit_real > (jam_kerja * 60),jam_kerja,jam_real) jam_real, if(menit_real > (jam_kerja * 60),(jam_kerja * 60),menit_real) menit_real, jam_kerja,plan_target from (SELECT a.sewing_line,sum(actual) actual,sum(min_prod) min_prod,man_power,jam_real,menit_real FROM (SELECT id,sewing_line,smv,actual,round(smv * actual,4) min_prod FROM (SELECT sewing_line,id,smv from master_plan where tgl_plan = '2023-06-26' ) a inner join 
(SELECT master_plan_id, count(a.id) actual from output_rfts a inner join master_plan b on b.id = a.master_plan_id where DATE_FORMAT(created_at, '%Y-%m-%d') = '2023-06-26' GROUP BY master_plan_id) b on b.master_plan_id = a.id) a inner join 
(SELECT sewing_line,man_power,jam_masuk,jam_sekarang, if(jam_kerja >= '6',jam_kerja -1,jam_kerja) jam_real, if(jam_kerja >= '6',menit_kerja -60,menit_kerja) menit_real from (SELECT sewing_line, man_power, CONCAT('2023-06-26',' ','07:00:00') jam_masuk,'2023-06-26' as jam_sekarang, TIMESTAMPDIFF(hour,CONCAT('2023-06-26',' ','07:00:00'),'2023-06-26 20:00:00') jam_kerja,TIMESTAMPDIFF(minute,CONCAT('2023-06-26',' ','07:00:00'),'2023-06-26 20:00:00') menit_kerja from master_plan where tgl_plan = '2023-06-26' GROUP BY sewing_line order by sewing_line asc ) a) b on b.sewing_line = a.sewing_line GROUP BY a.sewing_line) a inner join
...
(SELECT * from (SELECT b.sewing_line,count(a.id) deffect from output_defects a inner join master_plan b on b.id = a.master_plan_id where DATE_FORMAT(created_at, '%Y-%m-%d') = '2023-06-26' GROUP BY b.sewing_line) a order by a.deffect desc) a on a.sewing_line = line.username inner join 
